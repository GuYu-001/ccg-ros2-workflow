...
// ROS2 版本：frontend = 上层应用, backend = 底层控制
const ALL_COMMANDS = [
  'workflow', // 完整7阶段ROS2开发工作流（含硬件部署）
  'plan', // 多模型协作规划（Phase 1-2）
  'execute', // 多模型协作执行（Phase 3-5）
  'frontend', // 上层应用专项（Gemini主导：Launch/Python/RViz）
  'backend', // 底层控制专项（Codex主导：C++/硬件/实时）
  'feat', // 智能功能开发
  'analyze', // 技术分析
  'debug', // 问题诊断+修复
  'optimize', // 性能优化
...
// Workflow configurations (ROS2 版本)
